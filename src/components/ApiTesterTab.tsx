...
export default function ApiTesterTab() {
  ...
  const [isNewTestMeeting, setIsNewTestMeeting] = useState(false);
  ...
  useEffect(() => {
    if (selectedMeetingId && meetings.length > 0) {
      ...
      autoPopulateRoomConfig();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedMeetingId, meetings]);

  const autoPopulateRoomConfig = () => {
    const selectedMeeting = meetings.find(m => m.id === selectedMeetingId);
    if (!selectedMeeting) return;

    // Auto-populate room configuration from the selected meeting
    const meetingRoomType = (selectedMeeting.room_type as RoomType) || 'main';
    setRoomType(meetingRoomType);

    if (meetingRoomType === 'breakout' && selectedMeeting.room_number) {
      setRoomNumber(selectedMeeting.room_number);
    }

    // Mark as not a new test meeting (fields should be read-only)
    setIsNewTestMeeting(false);
  };
  ...
  const generatePowerShellCommand = () => {
    ...
    const roomInfo = roomType === 'breakout' ? `Breakout Room ${roomNumber}` : 'Main Conference Room';
    const meetingComment = `# Test for: ${selectedMeeting.topic || 'Untitled Meeting'}\n# Meeting UUID: ${selectedMeeting.meeting_uuid}\n# Room: ${roomInfo}\n\n`;

    return `${meetingComment}$headers = @{
      ...
      <div className="space-y-6">
            ...
            {selectedMeeting && (
              <div className="bg-white rounded-lg p-3 border border-slate-200 space-y-2">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <div className="text-xs text-slate-500 mb-1">Room Configuration</div>
                    <div className="text-sm text-slate-900">
                      {selectedMeeting.room_type === 'breakout'
                        ? `Breakout Room ${selectedMeeting.room_number || ''}`
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-medium text-slate-900">Room Configuration</h3>
            {!isNewTestMeeting && selectedMeeting && (
