  ...
  return (
    <div className="min-h-screen bg-slate-50">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-3 sm:py-0 sm:h-16 gap-3 sm:gap-0">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h1 className="text-base sm:text-lg font-bold text-slate-900 truncate">Ambient Intelligence</h1>
                <p className="text-xs text-slate-500 hidden sm:block">Orchestrator Control Panel</p>
              </div>
            </div>

            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
              <div className="flex items-center gap-3 sm:gap-6 text-xs sm:text-sm overflow-x-auto pb-1 sm:pb-0 -mx-4 px-4 sm:mx-0 sm:px-0">
                <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
                  <div className={`w-2 h-2 rounded-full ${stats.activeRooms > 0 ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                  <span className="text-slate-600 whitespace-nowrap">{stats.activeRooms} Active</span>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
                  <Brain className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-500" />
                  <span className="text-slate-600 whitespace-nowrap">{stats.totalTopics} Topics</span>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
                  <AlertTriangle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-500" />
                  <span className="text-slate-600 whitespace-nowrap">{stats.misalignments} Issues</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <button
                  ...
                  disabled={processing}
                  className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 sm:px-4 py-2.5 sm:py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors text-sm"
                ...
                <a
                  href="/display"
                  target="_blank"
                  className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 sm:px-4 py-2.5 sm:py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-colors text-sm"
                >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div className="flex gap-2 mb-4 sm:mb-6 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:overflow-visible">
          {[
            { id: 'prompts', label: 'Prompts', fullLabel: 'System Prompts', icon: Settings },
            { id: 'context', label: 'Context', fullLabel: 'External Context', icon: FileText },
            { id: 'status', label: 'Status', fullLabel: 'Room Status', icon: Radio },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as typeof activeTab)}
              className={`flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg font-medium transition-colors whitespace-nowrap flex-shrink-0 text-sm sm:text-base ${
                activeTab === tab.id
        ...
        {activeTab === 'context' && (
          <div className="space-y-4 sm:space-y-6">
            <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-semibold text-slate-900 mb-2 sm:mb-4">Upload Reference Documents</h3>
              <p className="text-xs sm:text-sm text-slate-500 mb-4">
                Upload PDFs, DOCX, or text files to provide external context for AI analysis.
              </p>

              <div className="border-2 border-dashed border-slate-200 rounded-lg sm:rounded-xl p-6 sm:p-8 text-center hover:border-blue-400 transition-colors active:border-blue-500 active:bg-blue-50/50">
                <input
                  ref={fileInputRef}
                  type="file"
                  accept=".pdf,.docx,.txt"
                ...
                <label htmlFor="file-upload" className="cursor-pointer">
                  ...
                  <p className="text-xs sm:text-sm text-slate-400 mt-1">PDF, DOCX, or TXT files</p>
                </label>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 overflow-hidden">
              <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100">
                <h3 className="font-semibold text-slate-900 text-sm sm:text-base">Uploaded Documents</h3>
              </div>
              ...
              {contextsLoading ? (
                ...
                <div className="divide-y divide-slate-100">
                  {contexts.map((ctx, index) => (
                    <div key={`${ctx.file_name}-${index}`} className="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="font-medium text-slate-900 text-sm sm:text-base truncate">{ctx.title}</p>
                      ...
                      <button
                        ...
                        className="p-2.5 sm:p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0"
                      >
        ...
        {activeTab === 'status' && (
          <div className="space-y-4 sm:space-y-6">
            <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-semibold text-slate-900 mb-2">Main Conference Room</h3>
              <RoomStatusCard
                roomNumber={0}
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-semibold text-slate-900 mb-3 sm:mb-4">Breakout Rooms</h3>
...
function RoomStatusCard({
  ...
  const roomName = roomNumber === 0 ? 'Main Room' : `Room ${roomNumber}`;
  const shortName = roomNumber === 0 ? 'Main' : `R${roomNumber}`;
  ...
  return (
    <div className={`rounded-lg sm:rounded-xl border p-3 sm:p-4 ${
      ...
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        ...
        {isActive && <Radio className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-500" />}
      </div>

      {summary ? (
        <div className="space-y-1.5 sm:space-y-2">
          <p className="text-xs sm:text-sm text-slate-600 line-clamp-2">{summary.content}</p>
          {summary.key_topics && summary.key_topics.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {(summary.key_topics as string[]).slice(0, 2).map((topic, i) => (
                <span key={i} className="px-1.5 sm:px-2 py-0.5 bg-white text-[10px] sm:text-xs text-slate-600 rounded-full border border-slate-200 truncate max-w-[80px] sm:max-w-none">
                  {topic}
                </span>
              ))}
            </div>
